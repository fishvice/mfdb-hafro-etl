...
mdb <- mfdb('Iceland', destroy_schema = TRUE,db_params = list(host='mfdb.hafro.is'))
...
mfdb_import_survey(mdb,
                   ...
                   aldist %>%
                     filter(!(tow %in% c(1e5,4e5))) %>% # weird tows
...
landed_catch <-
  ...
  dplyr::union_all(fiskifelag_oslaegt(mar) %>%
                     select(-flokkur) %>%
...
tbl(mar,'ices_landed_catch') %>%
  filter(species_code %in% c('LIN','USK'),
         trim(area) %in% c("5"),
         country != 'Iceland',
         trim(subarea) %like% 'a%',
         year %in% 1966:2013) %>%
  ...
  distinct() %>%
  collect(n=Inf) %>%
  #bind_rows(tibble(year=2013,
  #                 species_code=c('LIN','USK'),
  #                 landings = c(1324, 1284))) %>%
  left_join(expand.grid(year=1966:2015,month=1:12)) %>%
...
mfdb_share_with(mdb,c('pamela','anika','ella','warsha','krik','magnus'))
